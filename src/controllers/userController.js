...
const userLogin = async function(req, res) {
    try {
      ...
      if(!isValidEmail(email)){return res.status(400).send({ status: false, msg: " Email-Id is invalid"})};

      if(!password) return res.status(400).send({ status: false, msg: "Password Required." });
      if(!isValidPassword(password)){return res.status(400).send({ status: false, essage: "Password should be minLen 8, maxLen 15 long and must contain one of 0-9,A-Z,a-z & special char", })}
...
///------------------------- generate jwt token--------------------//
        let token = jwt.sign({userId: user._id, iat: Math.floor(Date.now())}, "group21",{ expiresIn:"10h"});
